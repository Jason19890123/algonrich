...
const WBNB_CONTRACT = '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c' // contract for WBNB
...
export const useTrade = (network: number) => {
  ...
  const [bnbalgoRate, setbnbalgoRate] = useState(1);
  ...
  const getTradeAmount = useCallback(async (amountIn: number, token0: string, token1: string) => {
    ...
    try {
      ...
      if (pair) {
        ...
        setbnbalgoRate(Number(formatEther(amounts[1])) / Number(formatEther(amounts[0])));
      }
    } catch (e) {
      console.log(e)
      setError(e as Record<string, any>);
  ...
  return { getTradeAmount, update, error, bnbalgoRate, tradeAmount };
}
    ...
    async (amount: number, coinFrom: string, coinTo: string) => {
      ...
      switch (coinTo) {
        case "BNB":
          ...
          break;
        case "ALGO":
          token1 = ALGO_CONTRACT;
          break;
        default:
          token1 = ALGO_CONTRACT;
          break;
      }
      if (!account || isWrongNetwork || (token0 === token1))
        return false;
      const wei = parseEther(String(amount));
      const contract = GetContract(PANCAKE_ROUTER_CONTRACT, PANCAKE_ROUTER_ABI, library);
      if (coinFrom === "BNB") {
        try {
          const tx = await contract.swapExactETHForTokens(0, [token0, token1], account, Date.now() + 1000 * 60, { value: wei })
          await tx.wait();
        ...
        try {
          const tx = await contract.swapExactTokensForTokens(wei, 0, [token0, token1], account, Date.now() + 1000 * 60)
          await tx.wait();
          toastSuccess('Your request successed');
        } catch (error) {
          console.log(error)
